...
class DashboardHeader extends Component {
  ...
  renderName = user => (
    <div id='header__userinfo'>
      ...
      />
      <div>{user[0].loginName}</div>
    </div>
  );

  renderSignIn = () => (
    <div id='header__sign-in'>
      {' '}
      Please,
      <a
    ...
    <form action='#' className='header__search-form'>
      ...
      <button className='header__search-button'>
        <svg className='header__search-icon'>
          <use xlinkHref='./icons/sprite.svg#icon-zoom-outline'>
            <title />
          </use>
        </svg>
      </button>
      ...
      <header className='dashboard__header'>
        <div className='header__logo'>
          <svg className='header__logo-icon'>
            <use xlinkHref='./icons/sprite.svg#icon-social-github-circular' />
          </svg>
        </div>
